...
projected stats. In the `ffwebpackage` scoring settings are defined by groups of
variables.
...
is described seprarately, but each of the variable groups are also list, The list
elemensts are named by the variables used in the scoring and the values is the 
...
within the group are valid for all positions. 
...
If scoring settings are not specified when calclulating the projected points, the
